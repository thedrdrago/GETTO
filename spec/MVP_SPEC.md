...
| `id` | UUID | Primary key |
| `project_id` | UUID | Foreign key → projects |
| `area_id` | UUID | Foreign key → areas (optional grouping) |
...
**Table: `areas` (Optional for MVP, useful for grouping)**
| Field | Type | Description |
|---|---|---|
| `id` | UUID | Primary key |
| `project_id` | UUID | Foreign key → projects |
| `name` | VARCHAR(255) | Area name (e.g., "Ground Floor", "Roof") |
| `created_at` | TIMESTAMP | Creation timestamp |
...
- [ ] Area/zone grouping (organize drawings)
...
### Search
- `GET /api/search?q={query}&status={status}&area={area}` – Search drawings
